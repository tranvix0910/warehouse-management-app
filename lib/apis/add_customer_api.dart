    ...
    try {
      ...
      if (e.response != null) {
        ...
        if (statusCode == 401) {
          ...
          throw Exception(errorData['message'] ?? 'Invalid customer data provided.');
        } else if (statusCode == 404) {
          throw Exception('Customers endpoint not found.');
        } else if (statusCode == 409) {
          throw Exception(errorData['message'] ?? 'Customer already exists.');
        } else if (statusCode! >= 500) {
          throw Exception('Server error. Please try again later.');
        }

        throw Exception(errorData['message'] ?? 'Failed to create customer ($statusCode)');
      }
